...
int main() {
    ...
    */
    ...
    //result = getStringStackBipartitions(newickString2);
    result = getAllTaxons(newickString2);

    string stringTest = "bat:31, cow:22, goat:21, bird:12";
    vector<string> vectorTest = splitStringToVector(stringTest);

    for(string s: result) {
        cout << s << endl;
    }

    pair<list<string>, list<string>> bipartitionTest;

    bipartitionTest = createPairBipartition(result,vectorTest);

    cout << "\n\n\nFIRST : " << endl;
    for (string s: bipartitionTest.first) {
        cout << s << endl;
    }
    cout << "SECOND : " << endl;
    for (string s: bipartitionTest.second) {
        cout << s << endl;
    }
